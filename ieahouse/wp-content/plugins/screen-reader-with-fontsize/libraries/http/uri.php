...
class JUri {
	...
	public static function getInstance($uri = 'SERVER') {
		if (empty ( self::$instances [$uri] )) {
			...
			if ($uri == 'SERVER') {
				...
				/*
				 * Since we are assigning the URI from the server variables, we first need to determine if we are running on apache or IIS. If PHP_SELF and REQUEST_URI are present, we will assume we are running on apache.
				 */
				...
				if (! empty ( $_SERVER ['PHP_SELF'] ) && ! empty ( $_SERVER ['REQUEST_URI'] )) {
					...
					/*
					 * Since we do not have REQUEST_URI to work with, we will assume we are running on IIS and will therefore need to work some magic with the SCRIPT_NAME and QUERY_STRING environment variables. IIS uses the SCRIPT_NAME variable instead of a REQUEST_URI variable... thanks, MS
					 */
	...
	public function parse($uri) {
		...
		/*
		 * Parse the URI and populate the object fields. If URI is parsed properly, set method return value to true.
		 */
		$parts = parse_url ( $uri );
	 ...
	 *
	 * @param array $parts
	 *        	An array specifying the parts to render.
	 *        	
	 ...
	 *
	 * @param string $name
	 *        	Name of the query variable to get.
